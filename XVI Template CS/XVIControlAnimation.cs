...
public abstract partial class XVIControlAnimation : GodotObject {
    ...
    private static void KillTween( Control control ) {
        if ( !control.HasMeta( MetadataTween) ) return;
        
        Tween tween = (Tween)control.GetMeta( MetadataTween );
        if ( tween.IsRunning() ) tween.Kill();
    }
    ...
    public static Tween CloseWindow( Control control, double duration = 0.125 ) {
        ...
        tween.TweenProperty( control, PathModulate, Colors.Transparent, duration );
        tween.TweenProperty( control, PathOffsetTransScale, ClosedWindowScale, duration );
        tween.TweenCallback( Callable.From( control.Hide ) ).SetDelay( duration );
    ...
    public static void OpenWindow( Control control, bool stageNode = true, double duration = 0.125 ) {
        ...
        tween.TweenProperty( control, PathModulate, Colors.White, duration );
        tween.TweenProperty( control, PathOffsetTransScale, Vector2.One, duration );
